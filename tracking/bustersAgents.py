    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghosts = [ghost.argMax() for ghost in livingGhostPositionDistributions]
        distances = []
        actions = []
        ...
        for position in ghosts:
            for action in legal:
                ...
                possible = Actions.getSuccessor(pacmanPosition, action)
                distance = self.distancer.getDistance(possible, position)
