...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for domain in [GHOST0, PAC, GHOST1]:
        variableDomainsDict[domain] = positions

    distances = []
    for a in positions:
        for b in positions:
            manhattanDist = manhattanDistance(a, b)
            if abs(manhattanDist + MAX_NOISE) not in distances:
                distances.append(abs(manhattanDist + MAX_NOISE))
            if abs(manhattanDist - MAX_NOISE) not in distances:
                distances.append(abs(manhattanDist - MAX_NOISE))

    for obstacle in [OBS0, OBS1]:
        variableDomainsDict[obstacle] = distances
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            allFactors, join = joinFactorsByVariable(allFactors, var)
            if len(join.unconditionedVariables()) != 1:
                join = eliminate(join, var)
                allFactors.append(join)
        update = joinFactors(allFactors)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        keys = list(self.keys())
        totalKeys = self.total()
        runningTotal = 0
        rand = random.random() * totalKeys
        for key in keys:
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        discreteD = DiscreteDistribution()
        for prev in self.allPositions:
            for next in self.getPositionDistribution(gameState, prev).keys():
                discreteD[next] += self.beliefs[prev] * self.getPositionDistribution(gameState, prev)[next]
        self.beliefs = discreteD
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            discreteD[particle] += self.getObservationProb(observation, pacPos, particle, jailPos)
        updateParticles = []
        if discreteD.total() == 0:
            self.initializeUniformly(gameState)
        else:
            for i in range(self.numParticles):
                updateParticles += [discreteD.sample()]
    ...
    def elapseTime(self, gameState):
        ...
        sampleParticles = []
        for particle in self.particles:
            sampleParticles += [self.getPositionDistribution(gameState, particle).sample()]
        self.particles = sampleParticles
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for particle in range(self.numParticles):
            self.particles.append(permutations[particle%len(permutations)])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                prob = prob * self.getObservationProb(observation[ghost], pacPos, particle[ghost], self.getJailPosition(ghost))
            discreteD[particle] += prob
        updatedParticles = []
        if discreteD.total() == 0:
            self.initializeUniformly(gameState)
        else:
            for i in range(self.numParticles):
                updatedParticles += [discreteD.sample()]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for ghost in range(self.numGhosts):
                newDistribution = self.getPositionDistribution(gameState, oldParticle, ghost, self.ghostAgents[ghost])
                newParticle[ghost] = newDistribution.sample()
            """*** END YOUR CODE HERE ***"""
