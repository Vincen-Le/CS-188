...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.stateAction[(state, action)]
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in legalActions:
            newQVal = self.getQValue(state, action)
            optimalVal = max(optimalVal, newQVal)
        return optimalVal
    ...
    def computeActionFromQValues(self, state):
        ...
        if not legalActions:
            return None
        optimalQVal = self.computeValueFromQValues(state)
        possibleActions = []
        for action in legalActions:
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.stateAction[(state, action)] = (1-self.alpha) * currQ + self.alpha * (reward + self.discount * qPrime)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in featureDict.keys():
            qVal += self.weights[feature] * featureDict[feature]
        return qVal
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        difference = (reward + (self.discount * qPrime)) - currQ
